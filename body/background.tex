...
A 1975 study showed that the consumption of a small amount of an addictive substance (in this case, a milkshake on a test audience of dieters) paradoxically caused dieters to consume more food afterwards, in contrast to the control group of non-dieters who ate less \cite{herman1975restrained}. Similarly, many of us find that picking up our phones for a short break or otherwise directed task can often snowball into a longer-than-intended session which comprised of all manner of actions unrelated to why we originally picked up the phone. Playing "just one round" of a video game often ends up being anything but.
...
\begin{itemize}
    ...
    \item Interface interactions are often not animated, which is not only potentially very jarring for the user, but also may result in a well-known concept known as change blindness, in which a user does not notice that an element of the interface has indeed changed. This directly translates to a higher amount of cognitive effort expenditure on the part of the user.
    \item The app is purely mobile, so the user always has to be in the presence of a distracting device.
\end{itemize}
